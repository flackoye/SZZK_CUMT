...
前端静态页面可独立启动；数据导入功能需要后端同时运行。本地开发已内置 `/api` 到 `http://127.0.0.1:8000` 的代理默认值，干净克隆后无需创建 `.env.local`。如需连接其他后端地址，可将 `qianduan/.env.example` 复制为 `qianduan/.env.local` 后覆盖：
### 方式 B：双终端独立启动
### 方式 A：一键协同启动（推荐）
...
.env.local 不纳入 Git，仅用于覆盖默认地址；不要在 VITE_ 变量中放秘密信息。
...
- 前端默认将 `/api/*` 请求透明代理至 `http://127.0.0.1:8000/*`，无需额外环境文件；`.env.local` 仅用于覆盖默认地址。
...
- 前端开发服务器默认将 `/api/*` 反向代理至 `http://127.0.0.1:8000/*`；需要其他地址时再通过 `.env.local` 覆盖。
| 脚本命令 | 说明 |
| :--- | :--- |
